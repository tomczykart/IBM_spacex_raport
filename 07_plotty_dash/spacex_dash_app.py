...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        data = spacex_df
        fig = px.pie(data, values='class', names='Launch Site', title='Landing Site total', hole = 0.7)
        return fig
    else:
        mask = (spacex_df['Launch Site'] == entered_site)
        # return the outcomes piechart for a selected site
        #max_payload = spacex_df['Payload Mass (kg)'].max()
        data = spacex_df[mask].groupby(['Launch Site', 'class']).size().reset_index(name='class count')
        #data = data[['class']].count()
        
        fig = px.pie(data, values='class count', 
        names='class', 
        title=f'Succesful and failed launches for {entered_site}',
        hole = 0.7)
        return fig
...
def get_scatter(entered_site, slider_range):
    ...
    if entered_site == 'ALL':
        data = spacex_df[mask]
        fig = px.scatter(data, x='Payload Mass (kg)', y='class', color='Booster Version Category', title='Landing Site total')
        return fig
        ...
        data = spacex_df[mask]
        fig = px.scatter(data, x='Payload Mass (kg)', y='class', color='Booster Version Category', title=f'Succesful and failed landings for {entered_site}')
        return fig
